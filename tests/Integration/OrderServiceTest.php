...
class OrderServiceTest extends WP_UnitTestCase
{
    ...
    public function test_link_orders_by_email_links_guest_orders()
    {
        ...
        $userId = self::factory()->user->create(['user_email' => $email]);

        $linked = $this->service->linkOrdersByEmail($email);

        $this->assertEquals(2, $linked);
        $this->assertEquals($userId, wc_get_order($order1->get_id())->get_customer_id());
        $this->assertEquals($userId, wc_get_order($order2->get_id())->get_customer_id());
    }

    public function test_link_orders_by_email_returns_zero_for_unknown_email()
    {
        $linked = $this->service->linkOrdersByEmail('nobody@example.com');

        $this->assertEquals(0, $linked);
    }

    public function test_link_orders_by_email_returns_zero_for_invalid_email()
    {
        $linked = $this->service->linkOrdersByEmail('not-an-email');

        $this->assertEquals(0, $linked);
    }

    public function test_link_orders_by_email_skips_already_linked_orders()
    {
